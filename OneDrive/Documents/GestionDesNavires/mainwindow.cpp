    ...
    , nextId(1)
{
    ...
    // Dropdown menu for "Gestion Des Navires"
    menuGestionNavires = new QMenu(ui->btnGestionNavires);
    menuGestionNavires->setObjectName("menuGestionNavires");
    menuGestionNavires->setStyleSheet(R"(
        QMenu {
            background: #0d1620;
        ...
        QMenu::item:selected {
            background: rgba(63, 193, 249, 0.08);
            color: #cfd8dc;
        }
        QMenu::item:checked {
            color: #3fc1f9;
    ...
    menuGestionNaviresGroup = new QActionGroup(menuGestionNavires);
    menuGestionNaviresGroup->setExclusive(true);

    actionNaviresAffichage = menuGestionNavires->addAction("Affichage");
    actionNaviresStatistique = menuGestionNavires->addAction("Statistique");
    actionNaviresAvancees = menuGestionNavires->addAction(QStringLiteral("Avanc\u00e9es"));
    for (QAction *a : {actionNaviresAffichage, actionNaviresStatistique, actionNaviresAvancees}) {
        a->setCheckable(true);
        menuGestionNaviresGroup->addAction(a);
    }
    actionNaviresAffichage->setChecked(true);

    connect(actionNaviresAffichage, &QAction::triggered, this, [this]() {
        setNaviresAffichageOnly();
    });
    connect(actionNaviresStatistique, &QAction::triggered, this, [this]() {
        setNaviresStatsOnly(true);
    });
    connect(actionNaviresAvancees, &QAction::triggered, this, [this]() {
        setNaviresAdvancedOnly(true);
    });

    // Show menu on press and keep sidebar selection behavior.
    connect(ui->btnGestionNavires, &QPushButton::pressed, this, [this]() {
        ...
        const QPoint pos = ui->btnGestionNavires->mapToGlobal(QPoint(0, ui->btnGestionNavires->height()));
        menuGestionNavires->popup(pos);
    });
