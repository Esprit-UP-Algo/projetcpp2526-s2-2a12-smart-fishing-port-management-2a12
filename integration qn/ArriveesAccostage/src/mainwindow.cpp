...
{
    if (ui->sideBtn1)
        connect(ui->sideBtn1, &QPushButton::clicked, this, &MainWindow::onGestionNaviresClicked);
...
void MainWindow::onGestionNaviresClicked()
{
    // Keep the Quai interface selected in this window, and launch the Navires app.
    if (ui->sideBtn2)
        ui->sideBtn2->setChecked(true);
    if (ui->sideBtn1)
        ui->sideBtn1->setChecked(false);

    const QString exeName =
#if defined(Q_OS_WIN)
        QStringLiteral("appGestionDesNavires.exe");
#else
        QStringLiteral("appGestionDesNavires");
#endif

    const QString exePath = QDir(QCoreApplication::applicationDirPath()).filePath(exeName);
    if (!QFileInfo::exists(exePath))
    {
        QMessageBox::warning(this,
                             tr("Gestion Des Navires"),
                             tr("Impossible de trouver l'application des navires:\n%1\n\n"
                                "Assurez-vous d'avoir compilé les deux projets (integration qn).")
                                 .arg(exePath));
        return;
    }

    if (!QProcess::startDetached(exePath, {}, QCoreApplication::applicationDirPath()))
    {
        QMessageBox::warning(this,
                             tr("Gestion Des Navires"),
                             tr("Impossible de lancer l'application des navires."));
    }
}
