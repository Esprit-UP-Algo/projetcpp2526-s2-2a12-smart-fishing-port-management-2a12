...
protected:
    bool eventFilter(QObject *watched, QEvent *event) override
    {
        ...
        // Global cursor tracking: ONLY closes when leaving the button/menu.
        // Do NOT auto-open on global mouse moves because popup menus can overlap
        // other sidebar buttons (opening should happen on actual button hover/click).
...
class ClickToDeselectSingleRowFilter final : public QObject
{
public:
    explicit ClickToDeselectSingleRowFilter(QTableView *table)
        : QObject(table)
        , m_table(table)
    {
...
protected:
    bool eventFilter(QObject *watched, QEvent *event) override
    {
        ...
        if (rows.size() == 1 && rows.front().row() == idx.row())
        {
            sel->clearSelection();
            sel->setCurrentIndex(QModelIndex(), QItemSelectionModel::NoUpdate);
            return true; // consume: prevent default re-select
        }
...
public:
    ...
    void initStyleOption(QStyleOptionViewItem *option, const QModelIndex &index) const override
    {
        ...
        // Keep selection visible across the full row.
        // Remove the focus hint to avoid the dotted focus rectangle.
...
QuaiWindow::QuaiWindow(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::QuaiWindow)
{
    // When embedded as a static library, resources from the module might not be
    // pulled in unless explicitly referenced.
    Q_INIT_RESOURCE(quai_resources);

    ui->setupUi(this);

    {
        const QSqlDatabase db = QuaiConnection::createInstance().database();
        if (!db.isValid() || !db.isOpen())
        {
            if (!QuaiConnection::createInstance().createConnection())
            {
                QMessageBox::warning(this,
                                     tr("Connexion"),
                                     tr("Échec de la connexion à la base de données des quais."));
            }
        }
    }
...
void QuaiWindow::applyStyleNames()
{
...
void QuaiWindow::setupCardEffects()
{
...
void QuaiWindow::setupLogo()
{
    QPixmap pm(":/quai/logo_circle.png");
    if (pm.isNull())
        pm = QPixmap(":/quai/vision_sight.jpeg");
    if (!pm.isNull())
        ui->logo->setPixmap(cropAndMakeCircularLogo(pm, 96, 0));
}
...
void QuaiWindow::setupFonts()
{
    {
        QFont ft = ui->headerH1->font();
        ft.setFamily(QStringLiteral("Segoe UI"));
        ft.setPixelSize(28);
        ft.setBold(true);
        ft.setLetterSpacing(QFont::AbsoluteSpacing, 0.0);
...
void QuaiWindow::setupTable()
{
    ...
    new ClickToDeselectSingleRowFilter(m_table);
...
void QuaiWindow::setupStatWidgets()
{
...
void QuaiWindow::wireSignals()
{
    if (ui->sideBtn1)
        connect(ui->sideBtn1, &QPushButton::clicked, this, &QuaiWindow::onGestionNaviresClicked);
    ...
    connect(ui->btnCreate, &QPushButton::clicked, this, &QuaiWindow::onCreate);
    connect(ui->btnUpdate, &QPushButton::clicked, this, &QuaiWindow::onUpdate);
    connect(ui->btnDelete, &QPushButton::clicked, this, &QuaiWindow::onDelete);
    connect(ui->btnPdf, &QPushButton::clicked, this, &QuaiWindow::onExportPdf);
    ...
    connect(m_model, &QAbstractItemModel::rowsInserted, this, &QuaiWindow::refreshStats);
    connect(m_model, &QAbstractItemModel::rowsRemoved, this, &QuaiWindow::refreshStats);
    connect(m_model, &QAbstractItemModel::modelReset, this, &QuaiWindow::refreshStats);
    connect(m_model, &QAbstractItemModel::dataChanged, this, &QuaiWindow::refreshStats);

    connect(m_model, &QAbstractItemModel::rowsInserted, this, &QuaiWindow::refreshTraceability);
    connect(m_model, &QAbstractItemModel::rowsRemoved, this, &QuaiWindow::refreshTraceability);
    connect(m_model, &QAbstractItemModel::modelReset, this, &QuaiWindow::refreshTraceability);
    connect(m_model, &QAbstractItemModel::dataChanged, this, &QuaiWindow::refreshTraceability);

    connect(ui->revenuePeriod, &QComboBox::currentIndexChanged, this, &QuaiWindow::refreshStats);
    ...
    connect(m_model, &QAbstractItemModel::rowsInserted, this, &QuaiWindow::refreshMiniMap);
    connect(m_model, &QAbstractItemModel::rowsRemoved, this, &QuaiWindow::refreshMiniMap);
    connect(m_model, &QAbstractItemModel::modelReset, this, &QuaiWindow::refreshMiniMap);
    connect(m_model, &QAbstractItemModel::dataChanged, this, &QuaiWindow::refreshMiniMap);

    connect(m_searchProxy, &QAbstractItemModel::modelReset, this, &QuaiWindow::refreshMiniMap);
    connect(m_searchProxy, &QAbstractItemModel::layoutChanged, this, &QuaiWindow::refreshMiniMap);
    connect(m_searchProxy, &QAbstractItemModel::rowsInserted, this, &QuaiWindow::refreshMiniMap);
    connect(m_searchProxy, &QAbstractItemModel::rowsRemoved, this, &QuaiWindow::refreshMiniMap);

    connect(m_searchProxy, &QAbstractItemModel::modelReset, this, &QuaiWindow::refreshTraceability);
    connect(m_searchProxy, &QAbstractItemModel::layoutChanged, this, &QuaiWindow::refreshTraceability);

    connect(m_sortProxy, &QAbstractItemModel::modelReset, this, &QuaiWindow::refreshMiniMap);
    connect(m_sortProxy, &QAbstractItemModel::layoutChanged, this, &QuaiWindow::refreshMiniMap);
    connect(m_sortProxy, &QAbstractItemModel::rowsInserted, this, &QuaiWindow::refreshMiniMap);
    connect(m_sortProxy, &QAbstractItemModel::rowsRemoved, this, &QuaiWindow::refreshMiniMap);

    connect(m_sortProxy, &QAbstractItemModel::modelReset, this, &QuaiWindow::refreshTraceability);
    connect(m_sortProxy, &QAbstractItemModel::layoutChanged, this, &QuaiWindow::refreshTraceability);
...
void QuaiWindow::onGestionNaviresClicked()
{
    // Smooth switch handled by the host (single-window integration).
    if (ui->sideBtn1 || ui->sideBtn2)
    {
        QSignalBlocker b1(ui->sideBtn1);
        QSignalBlocker b2(ui->sideBtn2);
        if (ui->sideBtn1)
            ui->sideBtn1->setChecked(true);
        if (ui->sideBtn2)
            ui->sideBtn2->setChecked(false);
    }
...
void QuaiWindow::refreshMiniMap()
{
...
void QuaiWindow::refreshTraceability()
{
...
void QuaiWindow::onExportPdf()
{
    ...
    // UX requirement: do not open the save dialog unless a row is selected.
    if (selectedRows.isEmpty())
    {
        QMessageBox::information(this, tr("Exporter PDF"), tr("Pour l'export, sélectionnez une ligne du tableau (un quai)."));
        return;
    }

    // UX requirement: if multiple rows are selected (e.g. select-all), behave like Facturation.
    if (selectedRows.size() > 1)
    {
        QMessageBox::information(this, tr("Exporter PDF"), tr("Pour l'export, sélectionnez un seul quai."));
        return;
    }
    ...
    {
    case 0: {
        ok = PdfExporter::exportInvoiceToPdf(filePath, title, m_sortProxy, invoiceRow, &error);
        break;
    }
    case 1:
