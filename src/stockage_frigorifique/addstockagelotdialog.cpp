...
AddStockageLotDialog::AddStockageLotDialog(ChambreFroideModel *chambreModel, QWidget *parent)
    : QDialog(parent), m_chambreModel(chambreModel)
{
    setWindowTitle(tr("Créer un lot"));
    setMinimumWidth(450);
    buildUi();
    applyTheme();
    wireSignals();
...
void AddStockageLotDialog::buildUi()
{
    ...
    // Title
    auto *title = new QLabel(tr("Créer un lot"), this);
...
void AddStockageLotDialog::applyTheme()
{
    setStyleSheet(R"(
        QDialog {
            background: #0c1a29;
...
void AddStockageLotDialog::wireSignals()
{
    connect(m_sortieRenseignee, &QCheckBox::toggled, this, &AddStockageLotDialog::onSortieCheckChanged);
    connect(m_btnAjouter, &QPushButton::clicked, this, &AddStockageLotDialog::onAjouter);
    connect(m_btnEnregistrer, &QPushButton::clicked, this, &AddStockageLotDialog::onEnregistrer);
}

void AddStockageLotDialog::onAjouter()
{
    const StockageLot l = getStockageLot();
    if (m_callback && m_callback(l))
        clearForm();
}

void AddStockageLotDialog::onEnregistrer()
{
    accept();
}

void AddStockageLotDialog::clearForm()
{
    m_idLot->clear();
    m_espece->clear();
    m_quantite->setValue(0);
    m_entree->setDateTime(QDateTime::currentDateTime());
    m_sortie->setDateTime(QDateTime::currentDateTime());
    m_sortieRenseignee->setChecked(false);
}

void AddStockageLotDialog::setLotAddedCallback(std::function<bool(const StockageLot &)> callback)
{
    m_callback = callback;
}

void AddStockageLotDialog::onSortieCheckChanged(bool checked)
{
    m_sortie->setEnabled(checked);
}

StockageLot AddStockageLotDialog::getStockageLot() const
{
    StockageLot l;
