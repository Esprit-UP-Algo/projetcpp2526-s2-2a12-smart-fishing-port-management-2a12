...
void StatCircleWidget::paintEvent(QPaintEvent *)
{
    ...
    // center text (percentage or custom)
    QString centerText = m_centerText.isEmpty() 
        ? QString::number(int(m_value * 100 + 0.5)) + QStringLiteral(" %")
        : m_centerText;
    ...
    QFontMetrics fm(f);
    const int tw = fm.horizontalAdvance(centerText);
    p.drawText((w - tw) / 2, h / 2 + fm.ascent() / 2, centerText);
